...
type GameState = { id: string; hostId: string; judgingMode: string; phase: string; players: Array<{ id: string; score?: number }>; rounds: Array<{ number: number; prompt: string; artworks: Array<{ playerId: string; image?: string; votes?: number; [key: string]: unknown }>; [key: string]: unknown }>; promptIndex: number; roundEndsAt?: string; [key: string]: unknown };
...
export async function GET(request: NextRequest) {
  ...
  try { const game = await findRoom(id); return game ? NextResponse.json({ game }) : NextResponse.json({ error: "Room not found" }, { status: 404 }); }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const game = current ? mergeGame(current, incoming) : incoming;
