...
export default function Home() {
  const [game, setGame] = useState<Game | null>(null); const [me, setMe] = useState<string>(""); const [clock, setClock] = useState(() => Date.now()); const [notice, setNotice] = useState("");
  ...
  useEffect(() => { if (!game) return; const timer = window.setInterval(() => void fetchRoom(game.id).then(remote => { if (remote && JSON.stringify(remote) !== JSON.stringify(game)) setGame(remote); }), 2500); return () => clearInterval(timer); }, [game]);
  ...
  const seconds = game?.phase === "drawing" && game.roundEndsAt ? Math.max(0, Math.ceil((new Date(game.roundEndsAt).getTime() - clock) / 1000)) : 0;
  ...
  const start = () => { if (!game) return; const next = { ...game, roundSeconds: game.roundSeconds ?? 90, phase: "drawing" as const, rounds: [{ number: 1, prompt: PROMPTS[game.promptIndex], artworks: [] }], roundEndsAt: new Date(Date.now() + (game.roundSeconds ?? 90) * 1000).toISOString() }; save(next); };
  ...
  const vote = (targetId: string | null) => { if (!game) return; const round = game.rounds.at(-1)!; const mine = round.artworks.find(a => a.playerId === me); if (!mine || targetId === me || (mine as typeof mine & { voter?: boolean }).voter) return; const voted = { ...round, artworks: round.artworks.map(a => ({ ...a, votes: targetId && a.playerId === targetId ? (a.votes ?? 0) + 1 : (a.votes ?? 0) })) }; const ownArtwork = voted.artworks.find(a => a.playerId === me)!; (ownArtwork as typeof ownArtwork & { voter?: boolean }).voter = true; const allLocalVotes = voted.artworks.filter(a => (a as typeof a & { voter?: boolean }).voter).length >= game.players.filter(p => p.active).length; const pending = { ...game, rounds: [...game.rounds.slice(0, -1), voted] }; if (allLocalVotes) save(applyRoundScores(pending, rankArtworks(voted.artworks, a => a.votes ?? 0))); else save(pending); };
  const nextRound = () => { if (!game) return; const number = game.rounds.length + 1; save({ ...game, phase: "drawing", promptIndex: (game.promptIndex + 1) % PROMPTS.length, rounds: [...game.rounds, { number, prompt: PROMPTS[(game.promptIndex + 1) % PROMPTS.length], artworks: [] }], roundEndsAt: new Date(Date.now() + (game.roundSeconds ?? 90) * 1000).toISOString() }); };
  const playAgain = () => { if (!game) return; const promptIndex = (game.promptIndex + 1) % PROMPTS.length; const next = { ...game, phase: "drawing" as const, promptIndex, players: game.players.map(player => ({ ...player, score: 0, active: true })), rounds: [{ number: 1, prompt: PROMPTS[promptIndex], artworks: [] }], roundEndsAt: new Date(Date.now() + (game.roundSeconds ?? 90) * 1000).toISOString() }; save(next); };
...
function ArtworkGrid({ game, round, me, onVote }: { game: Game; round: Game["rounds"][number]; me: string; onVote?: (id: string | null) => void }) { return <div className="art-grid">{round.artworks.map((art, index) => { const owner = game.players.find(p => p.id === art.playerId); return <article className="art-card" key={art.playerId}><div className="art-meta">{art.rank && <b>#{art.rank}</b>}<span>{game.phase === "voting" ? `作品 ${index + 1}` : owner?.name}</span>{art.points !== undefined && <strong>+{art.points}pt</strong>}</div><img src={art.image} alt="提出作品" />{game.phase === "voting" && onVote && <button disabled={art.playerId === me} onClick={() => onVote(art.playerId)}>この作品に投票</button>}{game.phase !== "voting" && <div className="comment">{game.judgingMode === "ai" && <><b>{art.aiScore} 点</b><p>{art.comment}</p></>}{game.judgingMode === "player_vote" && <b>🗳️ {art.votes ?? 0} 票</b>}</div>}</article>; })}</div>; }
