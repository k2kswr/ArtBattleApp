...
function makeRoundSchedule(seconds: number) {
  ...
  return {
    roundStartsAt: new Date(start).toISOString(),
...
function useLiveDrawings(game: Game | null, me: string) {
  ...
  const [status, setStatus] = useState<"ready" | "connecting" | "unavailable">(
    "unavailable",
  );
  ...
  useEffect(() => {
    ...
    if (!roomId || !roundNumber || game?.phase !== "drawing") {
    ...
    const nextChannel = supabase
      .channel(roomChannelName(roomId, roundNumber), {
      ...
      .on("broadcast", { event: "draw" }, ({ payload }) => {
        if (
          !isDrawEvent(payload) ||
          payload.playerId === me ||
          !allowedPlayers.has(payload.playerId)
        )
          return;
        setEvents((previous) => [...previous, payload]);
      })
      .subscribe((subscriptionStatus) =>
        setStatus(subscriptionStatus === "SUBSCRIBED" ? "ready" : "connecting"),
      );
  ...
  const send = useCallback(
    (event: OutgoingDrawEvent) => {
      if (!channel.current) return;
      void channel.current.send({
        type: "broadcast",
        event: "draw",
        payload: { ...event, playerId: me },
      });
    },
    [me],
  );
...
function Canvas({
  ...
  onSubmit: (image: string) => Promise<boolean>;
  onDraw: (event: OutgoingDrawEvent) => void;
  expired: boolean;
}) {
  ...
  const [color, setColor] = useState("#2D1965");
  const [size, setSize] = useState(8);
  const [eraser, setEraser] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const point = (event: React.PointerEvent<HTMLCanvasElement>): DrawPoint => {
  ...
  const paint = (
    ...
    if (!canvas || expired || (!drawing.current && !start)) return;
    ...
    const strokeColor = eraser ? "#fffdf8" : color;
    const strokeSize = eraser ? size * 3 : size;
    drawSegment(
  ...
  const submit = async () => {
    if (submitted.current || submitting || !canvasRef.current) return;
    setSubmitting(true);
    const saved = await onSubmit(
      canvasRef.current.toDataURL("image/jpeg", 0.65),
    );
    if (saved) submitted.current = true;
    setSubmitting(false);
  };
  useEffect(() => {
    if (expired) void submit();
  }, [expired]);
  const stopDrawing = () => {
  ...
  return (
    <div className="canvas-wrap">
      <div className="toolbox">
        ...
        <button onClick={clear}>全部消す</button>
      </div>
      <canvas
        ref={canvasRef}
        width={900}
...
function LivePreviewGrid({
  ...
  const opponents = game.players.filter(
    (player) => player.active && player.id !== me,
  );
  ...
  return (
    <section className="live-previews">
      <div className="live-heading">
        <h2>👀 みんなの絵をのぞき見！</h2>
        <span
          className={status === "ready" ? "live-status ready" : "live-status"}
        >
          {status === "ready"
            ? "● ライブ接続中"
            : status === "connecting"
...
export default function Home() {
  ...
  const {
    ...
    send: sendDrawEvent,
    status: liveStatus,
  } = useLiveDrawings(game, me);
  const save = useCallback((next: Game) => {
    setGame(next);
    persist(next);
    ...
    void fetch("/api/game", {
      ...
      .then(async (response) => {
        ...
        if (requestId === latestSaveRef.current && body.game) {
          setGame(body.game);
          persist(body.game);
        }
      })
      .finally(() => {
        if (requestId === latestSaveRef.current) savingRef.current = false;
      });
  }, []);
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        if (!savingRef.current) {
          const remote = await fetchRoom(game.id, gameRef.current?.updatedAt);
          if (remote) {
            setGame(remote);
            persist(remote);
          }
  ...
  const waitingToStart =
    game?.phase === "drawing" &&
    !!game.roundStartsAt &&
    clock < new Date(game.roundStartsAt).getTime();
  const seconds =
    game?.phase === "drawing" && game.roundEndsAt
      ? waitingToStart
        ? game.roundSeconds
        : Math.max(
            0,
            Math.ceil((new Date(game.roundEndsAt).getTime() - clock) / 1000),
          )
      : 0;
  const create = (setup: Setup) => {
    ...
    localStorage.setItem("art-battle:me", playerId);
    setMe(playerId);
    save(next);
  };
  const join = async (id: string, name: string) => {
    const current = (await fetchRoom(id)) ?? loadGame();
    if (!current || current.id !== id) {
      setNotice("ルームが見つかりません。URLかルームIDを確認してください。");
      return;
    }
    if (current.phase !== "lobby" || current.players.length >= 8) {
      setNotice("このルームには今は参加できません。");
      return;
    }
    const playerId = makeId("p-");
    const next = {
      ...
      players: [
        ...current.players,
        { id: playerId, name, score: 0, active: true },
      ],
    };
    localStorage.setItem("art-battle:me", playerId);
    setMe(playerId);
  ...
  const submit = async (image: string): Promise<boolean> => {
    if (!game) return false;
    try {
      const response = await fetch("/api/game/submit", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ roomId: game.id, playerId: me, image }),
      });
      ...
      const local = {
        ...
        rounds: [
          ...
          {
            ...round,
            artworks: round.artworks.some((art) => art.playerId === me)
              ? round.artworks
              : [...round.artworks, { playerId: me, image }],
          },
        ],
      };
      setGame(local);
      persist(local);
      if (body.shouldJudge) {
        const remote = await fetchRoom(game.id);
        if (remote) {
          setGame(remote);
          persist(remote);
          void judge(remote);
      ...
      setNotice(
        "作品を保存できませんでした。通信状況を確認して、もう一度提出してください。",
      );
      return false;
    }
  };
  useEffect(() => {
    ...
    const missing = game.players.filter(
      (player) =>
        player.active &&
        !round.artworks.some((art) => art.playerId === player.id),
    );
    ...
    const expired = {
      ...
      rounds: [
        ...game.rounds.slice(0, -1),
        {
          ...
          artworks: [
            ...round.artworks,
            ...missing.map((player) => ({ playerId: player.id, image: blank })),
          ],
        },
      ],
    };
  ...
  const judge = async (pending: Game) => {
    try {
      ...
      const response = await fetch("/api/judge", {
        ...
        body: JSON.stringify({
          prompt: round.prompt,
          artworks: round.artworks,
        }),
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error);
      const ranked = rankArtworks(
        round.artworks.map((a) => ({
          ...a,
          aiScore:
            body.scores.find(
              (s: { playerId: string }) => s.playerId === a.playerId,
            )?.score ?? 0,
          comment:
            body.scores.find(
              (s: { playerId: string }) => s.playerId === a.playerId,
      ...
      save(applyRoundScores(pending, ranked));
    } catch (error) {
      setNotice(
        error instanceof Error ? error.message : "AI採点に失敗しました。",
      );
    }
  };
  const vote = (targetId: string | null) => {
    if (!game) return;
    const round = game.rounds.at(-1)!;
    const mine = round.artworks.find((a) => a.playerId === me);
    if (
      !mine ||
      targetId === me ||
      (mine as typeof mine & { voter?: boolean }).voter
    )
      return;
    const voted = {
      ...round,
      artworks: round.artworks.map((a) =>
        a.playerId === me
          ? {
              ...a,
              voter: true,
              votes:
                targetId && a.playerId === targetId
                  ? (a.votes ?? 0) + 1
                  : (a.votes ?? 0),
            }
          : {
    ...
    const targetName = targetId
      ? game.players.find((player) => player.id === targetId)?.name
      : null;
    setVoteMessage(
      targetName
        ? `${targetName}に投票しました！`
    ...
    const pending = { ...game, rounds: [...game.rounds.slice(0, -1), voted] };
    save(pending);
  };
  const nextRound = () => {
    if (!game) return;
    ...
    save({
      ...game,
      phase: "drawing",
      promptIndex: (game.promptIndex + 1) % PROMPTS.length,
      rounds: [
        ...game.rounds,
        {
          number,
  ...
  const finishGame = async () => {
    if (game) {
      const response = await fetch("/api/game", {
        method: "DELETE",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ id: game.id, hostId: me }),
      });
      if (!response.ok) {
        setNotice("ルームを削除できませんでした。もう一度お試しください。");
        return;
      }
    }
  ...
  if (!game)
    return (
      <>
        <SetupScreen onCreate={create} onJoin={join} />
  ...
  return (
    <main className="game">
      <header>
        ...
        <div className="score-pill">🏅 {player?.score ?? 0} pt</div>
      </header>
      {notice && (
        <div className="toast">
          {notice}
      ...
      {game.phase === "lobby" && (
        <section className="lobby panel">
          ...
          <h1>みんなを待っています！</h1>
          <p>この5桁のルームIDを友だちに伝えてください。</p>
          <div className="room-code">
            <strong>{game.id}</strong>
            <span>参加する人は初期画面で入力</span>
          </div>
          <div className="players">
            {game.players.map((p, i) => (
              <div className="player" key={p.id}>
                <span className={`avatar a${i % 5}`}>{p.name.slice(0, 1)}</span>
                {p.name}
                {p.id === game.hostId && <b>ホスト</b>}
              </div>
            ))}
          </div>
          {isHost && (
          ...
          {!isHost && (
            <p className="waiting">ホストが開始するのを待っています…</p>
          )}
        </section>
      )}
      {game.phase === "drawing" && round && (
        <section>
          ...
          <div className="drawing-layout">
            <div>
              {waitingToStart ? (
                <div className="start-countdown panel">
                  <h2>まもなくスタート！</h2>
                  <p>全員の画面が切り替わるまで少し待ってね。</p>
                </div>
              ) : round.artworks.some((a) => a.playerId === me) ? (
                <div className="submitted panel">
                  <h2>提出しました！</h2>
                  <p>
                    {round.artworks.length} / {active.length} 人が提出済みです。
                  </p>
                </div>
              ) : (
                <Canvas
                  onSubmit={submit}
                  onDraw={sendDrawEvent}
                  expired={seconds <= 0}
                />
              )}
            </div>
      ...
      {game.phase === "voting" && round && (
        <section className="vote">
          ...
          <ArtworkGrid game={game} round={round} me={me} onVote={vote} />
          <button className="secondary abstain" onClick={() => vote(null)}>
            今回は投票しない
          </button>
          {round.artworks.find(
            (art) =>
              art.playerId === me &&
              (art as typeof art & { voter?: boolean }).voter,
          ) && (
            <p className="vote-waiting">
              {voteMessage || "投票しました！"}
              <br />
              <span>みんなの投票を待っています・・・</span>
            </p>
          )}
        </section>
      )}
      {(game.phase === "results" || game.phase === "finished") && round && (
        <section className="results">
          <div className="round-head">
            <div>
              ...
              <h1>
                {game.phase === "finished" ? "🎉 優勝者の発表！" : "結果発表！"}
              </h1>
            </div>
          </div>
          ...
          <ArtworkGrid game={game} round={round} me={me} />
          {game.phase === "results" && isHost && (
            <button className="primary huge" onClick={nextRound}>
              つぎのラウンドへ →
            </button>
          )}
          {game.phase === "finished" && isHost && (
  ...
  return (
    <div className="art-grid">
      {game.players
        .filter((player) => player.active)
        .map((player) => {
          const art = artworks.get(player.id);
          if (!art) return null;
          return (
            <article className="art-card" key={art.playerId}>
              <div className="art-meta">
                {art.rank && <b>#{art.rank}</b>}
                <span>{player.name}</span>
                {art.points !== undefined && <strong>+{art.points}pt</strong>}
              </div>
              <img src={art.image} alt={`${player.name}の提出作品`} />
              {game.phase === "voting" && onVote && (
                <button
                  disabled={art.playerId === me}
                  onClick={() => onVote(art.playerId)}
                >
                  {player.name}に投票
                </button>
              )}
