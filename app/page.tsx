...
function Canvas({ onSubmit, onDraw, expired }: { onSubmit: (image: string) => Promise<boolean>; onDraw: (event: OutgoingDrawEvent) => void; expired: boolean }) {
  const canvasRef = useRef<HTMLCanvasElement>(null); const drawing = useRef(false); const lastPoint = useRef<DrawPoint | null>(null);
  const submitted = useRef(false);
  const [color, setColor] = useState("#2D1965"); const [size, setSize] = useState(8); const [eraser, setEraser] = useState(false); const [submitting, setSubmitting] = useState(false);
...
export default function Home() {
  ...
  const submit = async (image: string): Promise<boolean> => { if (!game) return false; try { const response = await fetch("/api/game/submit", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ roomId: game.id, playerId: me, image }) }); const body = await response.json() as { error?: string; shouldJudge?: boolean }; if (!response.ok) { setNotice(body.error ?? "作品を保存できませんでした。もう一度提出してください。"); return false; } const round = game.rounds.at(-1)!; const local = { ...game, rounds: [...game.rounds.slice(0, -1), { ...round, artworks: round.artworks.some(art => art.playerId === me) ? round.artworks : [...round.artworks, { playerId: me, image }] }] }; setGame(local); persist(local); if (body.shouldJudge) { const remote = await fetchRoom(game.id); if (remote) { setGame(remote); persist(remote); void judge(remote); } } return true; } catch { setNotice("作品を保存できませんでした。通信状況を確認して、もう一度提出してください。"); return false; } };
  useEffect(() => {
  ...
  const finishGame = async () => { if (game) { const response = await fetch("/api/game", { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: game.id, hostId: me }) }); if (!response.ok) { setNotice("ルームを削除できませんでした。もう一度お試しください。"); return; } } localStorage.removeItem(STORAGE); localStorage.removeItem("art-battle:me"); history.replaceState(null, "", window.location.pathname); setGame(null); setMe(""); };
