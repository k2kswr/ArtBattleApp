...
export default function Home() {
  ...
  useEffect(() => { if (!game || game.phase !== "drawing") return; if (seconds <= 0) return; const t = setTimeout(() => setSeconds(s => s - 1), 1000); return () => clearTimeout(t); }, [game, seconds]);
  ...
  const start = () => { if (!game) return; const next = { ...game, roundSeconds: game.roundSeconds ?? 90, phase: "drawing" as const, rounds: [{ number: 1, prompt: PROMPTS[game.promptIndex], artworks: [] }] }; setSeconds(next.roundSeconds); save(next); };
  ...
  const nextRound = () => { if (!game) return; const number = game.rounds.length + 1; save({ ...game, phase: "drawing", promptIndex: (game.promptIndex + 1) % PROMPTS.length, rounds: [...game.rounds, { number, prompt: PROMPTS[(game.promptIndex + 1) % PROMPTS.length], artworks: [] }] }); setSeconds(game.roundSeconds ?? 90); };
  const playAgain = () => { if (!game) return; const promptIndex = (game.promptIndex + 1) % PROMPTS.length; const next = { ...game, phase: "drawing" as const, promptIndex, players: game.players.map(player => ({ ...player, score: 0, active: true })), rounds: [{ number: 1, prompt: PROMPTS[promptIndex], artworks: [] }] }; setSeconds(next.roundSeconds ?? 90); save(next); };
